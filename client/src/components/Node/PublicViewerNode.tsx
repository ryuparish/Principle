...
interface PublicViewerNodeData {
  label: string;
  node: ConceptMapNode;
  onNodeClick?: (node: ConceptMapNode) => void;  // Callback for when node is clicked
}

const PublicViewerNode: React.FC<NodeProps<PublicViewerNodeData>> = ({ data, isConnectable }) => {
  ...
  return (
    <div
      className={`custom-node ${shapeConfig.cssClass || ''}`}
      onClick={handleClick}
...
export default PublicViewerNode;
